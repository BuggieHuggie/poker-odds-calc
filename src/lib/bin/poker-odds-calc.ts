...
argv.option([
  {
    ...
    type: "string",
    description:
      "Select between texas_holdem, shortdeck_holdem and omaha. Default texas_holdem.",
  },
  {
    name: "board",
    short: "b",
  ...
  },
  {
    ...
    type: "boolean",
    description:
      "Run all possible board combinations, regardless limit option.",
  },
  {
    name: "limit",
    short: "l",
...
let PrintResult = (result: Result) => {
  log();
  log(
    Log.color(`Board`, "grey") +
      "   " +
      table
        .getBoard()
        .getCards()
        .map((c) => Log.color(c.toString(), c.color()))
        .join(" ")
  );
  log();
  log(`Player        Hand         Wieens      Ties`, "red");
  result.getPlayers().forEach((player) => {
    log(
      `${Log.color(player.getName(), "cyan")}     ${(
        player.getPlayer().getCards() as Array<Card>
      )
        .map((c) => Log.color(c.toString(), c.color()))
        .join(" ")}     ${Log.color(
        player.getWinsPercentageString().padStart(7, " "),
        player.isWinner() ? "green" : "white"
      )}    ${Log.color(
        player.getTiesPercentageString().padStart(6, " "),
        "white"
      )}`
    );
  });
  ...
  if (!result.isApproximate()) {
    ...
    ranks.forEach((rank) => {
      let str = Log.color(
        players[0].getRanks()[rank].getName().padEnd(COL_WIDTH, " "),
        "grey"
      );
      players.forEach((player, i) => {
        if (player.getRanks()[rank].getCount() > 0)
          str += Log.color(
            ...
            player.getRanks()[rank].getCount() > 0 ? "white" : "grey"
          );
        else str += Log.color("_".padStart(COL_WIDTH, " "), "grey");
      });
      log(str);
    });
  }
...
try {
  ...
  if (options.tripsbeatstraight) table.tripsBeatsStraight();

  if (options.hasOwnProperty("limit")) table.limit(options.limit);

  if (options.exhaustive === true) table.exhaustive();
